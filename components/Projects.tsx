...
const Projects = () => {
	...
	const { showModal, setShowModal, setmodalType, modalType, userAuth } =
	...
	const totalProjects = projectData.reduce(
		(accumulator, project) => accumulator + Object.values(project).length,
		0
	);
	...
	return (
		<>
			<div className="flex items-center max-w-lg mx-auto sm:max-w-full justify-between flex-col sm:flex-row gap-5">
				<h1 className="text-xl md:text-2xl">{`Projects(${totalProjects})`}</h1>
